...
class MockAppConfigCollection extends Mock
    implements IsarCollection<AppConfig> {}
...
class MockQueryBuilder extends Mock
    implements QueryBuilder<VaultItem, VaultItem, QAfterFilterCondition> {}
...
void main() {
  ...
  setUp(() {
    ...
    when(
      () => mockIsar.collection<VaultItem>(),
    ).thenReturn(mockVaultCollection);
    when(
      () => mockIsar.collection<AppConfig>(),
    ).thenReturn(mockAppConfigCollection);
    when(() => mockIsar.collection<User>()).thenReturn(mockUserCollection);
    when(() => mockAppConfigCollection.get(any())).thenAnswer(
      (_) async => AppConfig()
        ..hasSeenDemo = true
  ...
  test(
    'VaultNotifier loads items from repository after initialization delay',
    () async {
      final items = [
        VaultItem()
          ..title = 'Test Item'
